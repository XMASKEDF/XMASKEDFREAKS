-- Aggregate referral activity and verified earnings for the Admin heatmap view.
-- Raw analytics never leaves this server-side function. Production and sandbox
-- activity are explicitly separated so test traffic cannot reach Admin reports.

create index if not exists analytics_events_heatmap_idx
  on public.analytics_events(environment, occurred_at desc, event_type);
create index if not exists admin_earnings_heatmap_idx
  on public.admin_earnings_ledger(status, verified_at desc, category);
...
begin
  if p_end <= p_start then
    raise exception 'INVALID_HEATMAP_RANGE';
  end if;
  ...
  with base_events as (
    select
      ...
      case lower(nullif(trim(e.source), ''))
        ...
        when 'external referral' then 'other'
  ...
  first_touch as (
    select distinct on (anonymous_session_hash)
  ...
  filtered_events as (
    select a.*,
      extract(isodow from (a.occurred_at at time zone v_timezone))::integer - 1 as day_index,
      floor(extract(hour from (a.occurred_at at time zone v_timezone)) / 2)::integer as bucket_index
    from attributed_events a
    where (lower(coalesce(p_source, 'all')) = 'all'
      or a.attributed_source = lower(trim(p_source))
      or a.referrer_host = lower(trim(p_source)))
      and (nullif(trim(coalesce(p_referrer_host, '')), '') is null
        or a.referrer_host = lower(trim(p_referrer_host)))
      and (lower(coalesce(p_page_section, 'all')) = 'all'
        or a.page_section = lower(trim(p_page_section)))
  ),
  visitor_rollup as (
    select
      day_index,
  ...
  visitor_cells as (
    ...
    from generate_series(0, 6) d(day_index)
    cross join generate_series(0, 11) b(bucket_index)
  ...
  purchase_rows as (
    select
      ...
      coalesce(linked.source_value, linked.referrer_host, 'unknown') as purchase_source,
      coalesce(linked.page_section, 'other') as purchase_page_section
    from public.admin_earnings_ledger l
    left join lateral (
      select a.source_value, a.referrer_host, a.page_section
      from attributed_events a
    ...
    where l.status = 'verified'
      and l.verified_at >= p_start
      and l.verified_at < p_end
      ...
      and (lower(coalesce(p_source, 'all')) = 'all'
        or coalesce(linked.source_value, linked.referrer_host, 'unknown') = lower(trim(p_source)))
      and (nullif(trim(coalesce(p_referrer_host, '')), '') is null
        or linked.referrer_host = lower(trim(p_referrer_host)))
      and (lower(coalesce(p_page_section, 'all')) = 'all'
        or coalesce(linked.page_section, 'other') = lower(trim(p_page_section)))
  ),
  purchase_rollup as (
